...
@section('content')
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1">
                    Modifique los parámetros técnicos o existencias del producto
                </p>
            </div>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST">
            ...
            {{-- Tarjeta 1: Información Base --}}
            <x-common.component-card title="Información Principal" class="mb-6">
                <div class="p-6">
                    {{-- Fila 1: Código y Nombre --}}
                    <div class="flex flex-col md:flex-row gap-6 mb-6">
                        <div class="w-full md:w-1/2">
                            <x-form.group name="code" label="Código de Producto (Solo Lectura)">
                                <div class="h-11 flex items-center rounded-lg border border-gray-200 bg-gray-50 px-4 text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-400 font-mono select-none">
                                    {{ $product->code }}
                                </div>
                        ...
                        <div class="w-full md:w-1/2">
                            <x-form.group name="name" label="Nombre del Producto" :required="true">
                                <x-form.input name="name" placeholder="Ej. Monitor LG 27''" :required="true" :value="old('name', $product->name)" />
                            </x-form.group>
                        </div>
                    </div>

                    {{-- Fila 2: Categoría y Marca (Premium Selects) --}}
                    <div class="flex flex-col md:flex-row gap-6 mb-6">
                    ...
                    {{-- Fila 3: Stocks y Unidad --}}
                    <div class="flex flex-col md:flex-row gap-6">
                        <div class="w-full md:w-1/3">
                            <x-form.group name="stock" label="Stock Actual" :required="true">
                                <x-form.input type="number" name="stock" :required="true" :value="old('stock', $product->stock)" />
                            </x-form.group>
                        </div>
                        <div class="w-full md:w-1/3">
                            <x-form.group name="stock_minimun" label="Stock Mínimo" :required="true">
                                <x-form.input type="number" name="stock_minimun" :required="true" :value="old('stock_minimun', $product->stock_minimun)" />
                            </x-form.group>
                        </div>
                        <div class="w-full md:w-1/3">
                            <x-form.group name="unit" label="Unidad" :required="true">
                                <x-form.input type="text" name="unit" placeholder="Caja, Unidad..." :required="true" :value="old('unit', $product->unit)" />
                            </x-form.group>
                        </div>
                    </div>
                </div>
